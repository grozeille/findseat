...
public class TeamDeskDispatcher {
    ...
    public WeekDispatchResult dispatch(Map<Integer, List<Team>> teamsByDay, List<Room> rooms) {
        ...
        for(int day = 1; day <= 5; day++) {
            ...
            boolean mandatoryNotDispatched = dayDispatchResult.getNotAbleToDispatch().stream()
                    .anyMatch(p -> p.getPeople().getBookingType().equals(BookingType.MANDATORY));

            boolean anyOptional = dayDispatchResult.getDeskAssignedToPeople().values().stream()
                    .anyMatch(p -> p.getPeople().getBookingType().equals(BookingType.OPTIONAL));

            boolean normalNotDispatched = dayDispatchResult.getNotAbleToDispatch().stream()
                    .anyMatch(p -> p.getPeople().getBookingType().equals(BookingType.NORMAL));
    ...
    private Pair<LinkedList<Team>, List<Team>> tryToFillTheFloor(LinkedList<Team> teams, Integer totalSizeForAllRooms) {
        ...
        if(totalSizeTeams <= totalSizeForAllRooms) {
            ...
            for (Team team : teams) {
                long countMandatory = team.getMembers().stream().filter(p -> p.getBookingType().equals(BookingType.MANDATORY)).count();
                long countAdditional = team.getMembers().stream().filter(p -> p.getBookingType().equals(BookingType.NORMAL)).count();
                long countOptional = team.getMembers().stream().filter(p -> p.getBookingType().equals(BookingType.OPTIONAL)).count();
                if (countMandatory > 0) {
                    // if the team contains non-mandatory members, keep them for later
                    if (countMandatory != team.getMembers().size()) {
                        if (countAdditional > 0) {
                            additionalPeopleInMandatoryTeams.put(team.getName(), team.getMembers().stream().filter(p -> p.getBookingType().equals(BookingType.NORMAL)).toList());
                        ...
                        if (countOptional > 0) {
                            optionalPeopleInMandatoryTeams.put(team.getName(), team.getMembers().stream().filter(p -> p.getBookingType().equals(BookingType.OPTIONAL)).toList());
                    ...
                    if (countAdditional != team.getMembers().size()) {
                        if (countOptional > 0) {
                            optionalPeopleInAdditionalTeams.put(team.getName(), team.getMembers().stream().filter(p -> p.getBookingType().equals(BookingType.OPTIONAL)).toList());
